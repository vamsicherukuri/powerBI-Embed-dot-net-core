...
namespace AppOwnsData.Services
{
    ...
    public class PbiEmbedService
    {
        ...
        /// <summary>
        /// Get Embed token with EffectiveIdentity for RLS
        /// </summary>
        /// <returns>Embed token</returns>
        public async Task<EmbedToken> GetEmbedTokenWithRLS(Guid reportId, IList<Guid> datasetIds, string username, IList<string> roles, [Optional] Guid targetWorkspaceId)
        {
            PowerBIClient pbiClient = await this.GetPowerBIClient();

            // Create EffectiveIdentity for RLS
            ...
            // Create a request for getting Embed token with EffectiveIdentity
            var tokenRequest = new GenerateTokenRequestV2(
                reports: new List<GenerateTokenRequestV2Report>() { new GenerateTokenRequestV2Report(reportId) },
                datasets: datasetIds.Select(datasetId => new GenerateTokenRequestV2Dataset(datasetId.ToString())).ToList(),
